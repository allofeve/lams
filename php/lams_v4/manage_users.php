...
<body>
        ...
        <div class="modal-dialog">
            <form id="edit_user_form" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">ระดับสิทธิ์ (Level)</label>
                        <select name="level_id" id="edit_level" class="form-select" required>
                            <?php
								$stmt = $conn->prepare("SELECT * FROM level_tb");
								$stmt->execute();
								while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $(document).ready(function() {
        ...
        function loadUsers() {
            $.get('user_process.php?action=list', function(data) {
                ...
                data.forEach(function(item) {
                    let badge = item.level_num == 99 ? 'bg-danger' : 'bg-primary';
                    html += `<tr>
                        <td>${item.id}</td>
                        <td class="text-start fw-bold">${item.username}</td>
                        <td><span class="badge ${badge}">Level ${item.level_num}</span></td>
                        <td>
                            <button onclick="editUser(${item.id}, '${item.username}', ${item.level_id})" class="btn btn-sm btn-warning">
        ...
        // ฟังก์ชันแก้ไข
        window.editUser = function(id, username, level_id) {
            $('#edit_id').val(id);
            $('#edit_username').val(username);
            $('#edit_level').val(level_id);
