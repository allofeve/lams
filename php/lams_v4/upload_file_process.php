...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$member_group = isset($_POST["member_group"]) ? trim($_POST["member_group"]) : "";
    $loan_date_th = isset($_POST["loan_date"]) ? trim($_POST["loan_date"]) : "";

    // ตรวจสอบเบื้องต้น
    if(empty($member_id) || empty($member_name) || empty($loan_id) || empty($member_group) || empty($loan_date_th)) {
        ...
        if ($file_ext != "pdf") {
            ...
            try {
                ...
                $sql = "INSERT INTO loan_tb (member_id, member_name, loan_id, loan_date_en, member_group, file_name) 
                        VALUES (:mid, :mname, :lid, :ldate, :mgroup, :fname)";
