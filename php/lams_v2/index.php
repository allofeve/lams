...
$allowed_pages = array();
$stmt_grant = $conn->prepare("SELECT page_name FROM grant_tb WHERE level <= ?");
$stmt_grant->execute(array($_SESSION['level']));
    ...
    <div class="container">
        <div class="card main-card">
            <div class="card-header bg-primary text-white text-center py-4" style="border-radius: 1rem 1rem 0 0;">
                <h1 class="h4 mb-0">ระบบจัดเก็บสัญญากู้ยืมเงิน</h1>
                <div class="mt-2 small text-white-100">
					<i class="fas fa-user-circle"></i> <?php echo $_SESSION['username']; ?> (Level: <?php echo $_SESSION['level']; ?>)
            ...
            <div class="card-body p-4">
                <div class="row">
                    ...
                    $menus = [
                        ...
                        ['file' => 'manage_grants.php', 'icon' => 'fa-user-shield', 'color' => 'text-dark', 'title' => 'จัดการสิทธิ์เข้าถึง'],
            ...
            if(data.length > 0) {
                data.forEach(function(u) {
                    html += `<span class="badge bg-white text-dark border p-2">
                                ...
                                <small class="text-muted">(Lv.${u.level})</small>
                             </span>`;
                });
            } else {
